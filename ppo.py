...
class Agent(nn.Module):
    ...
    # Overloaded get_action_and_value with additional argument
    # to allow invalid action masking
    def get_action_and_value(self, next_obs, action_mask, action=None):
        logits = self.actor(next_obs)
        #print(f"logits.shape is {logits.shape}")
        #print(f"action_mask.shape is {action_mask.shape}")

        # These are already the same shape.  I don't think splitting them is necessary
        # split_logits = torch.split(logits, envs.single_action_space.n) #self.nvec.tolist(), dim=1)
        # split_action_masks = torch.split(action_mask, envs.single_action_space.n) #self.nvec.tolist(), dim=1)
        
        multi_categoricals = [
            CategoricalMasked(logits=logits, masks=iam) for (logits, iam) in zip(logits, action_mask)
        ]
        if action is None:
            action = torch.stack([categorical.sample() for categorical in multi_categoricals])
        logprob = torch.stack([categorical.log_prob(a) for a, categorical in zip(action, multi_categoricals)])
        entropy = torch.stack([categorical.entropy() for categorical in multi_categoricals])
        return action, logprob.sum(0), entropy.sum(0), self.critic(next_obs)
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            masks = [env.action_mask() for env in envs.envs]
            #print(f"Any can be run? {[any(mask) for mask in masks]}")
            action_masks[step] = torch.Tensor(np.array(masks))

            # ALGO LOGIC: action logic
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs, action_masks[step])
                # print(f"step {global_step}: Taking actions {action}")
